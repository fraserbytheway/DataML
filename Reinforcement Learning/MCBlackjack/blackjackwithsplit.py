...
DECK = [Cards.ACE, 2, 3, 4, 5, 6, 7, 8, 9, 10, Cards.KING, Cards.QUEEN, Cards.JACK] * 4


def default_q():
    return {"count": 0, "success": 0.0, "wins": 0}
...
class Player(Person):
    def __init__(self, exp_rate=0.2, learn_rate=0.1, action_ref=None):
        super().__init__()
        self.states = []
        self.actions = [True, False]
        self.exploration_rate = exp_rate
        self.lr = learn_rate
        self.split_states = None
        self.split_start = []
        ...
        self.q_values = defaultdict(default_q)
        self.split_q_values = defaultdict(default_q)
    ...
    def action(self, d_total):
        ...
        if self.action_ref:
            self.stand = max(self.action_ref, key=lambda x: self.q_values[(state, x)]["success"])
    ...
    def reward(self, loss):
        ...
        for visits in set(self.states):
            ...
            if update == 1:
                self.q_values[visits]["wins"] += 1

    # Check if initial deal gave duplicate cards
    def check_duplicate(self):
        if len(self.hand) == 2 and self.hand[0] == self.hand[1]:
            return True
        return False

    # for finding optimal policy, only allow splitting on first hand
    def split_action(self, d_total):
        split_state = (tuple(self.hand), d_total)

        # epsilon soft mc policy
        if random.uniform(0, 1) < self.exploration_rate:
            action = random.choice(self.actions)
        else:
            action = max(self.actions, key=lambda x: self.split_q_values[(split_state, x)]["success"])

        # only track first split result
        if not self.split_states:
            self.split_states = (split_state, action)

        return action

    def split_reward(self, outcomes):
        ...
        for result in outcomes:
            if result is None:
                ...
                update += 1
        update = update / len(outcomes)

        self.split_q_values[self.split_states]["count"] += 1
        q = self.split_q_values[self.split_states]["success"]
        self.split_q_values[self.split_states]["success"] = q + self.lr * (update - q)

        if update > 0:
            self.split_q_values[self.split_states]["wins"] += 1
...
class Game:
    def __init__(self, player=None, num_decks=6, penetration=0.8):
        self.outcomes = None
        ...
        self.deck = deque()
        self.pen = 1 - penetration
        self.split = False
        self.split_A = False
        self.game = 0

        # used to initialise deck
        self.deck_check()
    ...
    def deck_check(self):
        if len(self.deck) < len(DECK) * self.num_decks * self.pen:
    ...
    def play(self):
        ...
        if self.player.check_duplicate():

            self.outcomes = []
            if self.player.split_action(self.dealer.hand[0]) == True:
                hands = [[self.player.hand[0]], [self.player.hand[1]]]
                if hands[0][0] == Cards.ACE:
                    self.split_A = True
                for hand in hands:
                    self.player.hand = hand
                    self.outcomes.append(self.game_loop())
            else:
                self.outcomes.append(self.game_loop())

            self.player.split_reward(self.outcomes)
            self.split = False
...
def merge_q_values(q_dicts):
    merged = defaultdict(default_merge_q)
    ...
    for q in q_dicts:
        for (state_action), data in q.items():
            ...
            merged[state_action]['wins'] += data["wins"]

    return merged


class Simulation:
    def __init__(self, exp_rate=0.2, learn_rate=0.1, action_ref=None, out='reg'):
        self.player = Player(exp_rate=exp_rate, learn_rate=learn_rate, action_ref=action_ref)
    ...
    def trials(self, n=10000):
        ...
        if self.out == 'reg':
            ...
            return self.player.split_q_values
